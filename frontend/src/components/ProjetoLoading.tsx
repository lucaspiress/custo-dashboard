...
export function ProjetosCarregando() {
  return (
    <div aria-busy="true" aria-label="Carregando projetos" className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {[0, 1, 2].map((indice) => (
        <div key={indice} className="rounded-xl border border-borda bg-superficie p-5 space-y-4">
          ...
          <div className="flex gap-2 border-t border-[#1a2138] pt-4">
...
function CabecalhoCarregando() {
  return (
    <header className="min-h-[64px] shrink-0 border-b border-[#1a2138] flex items-center justify-between gap-4 px-4 sm:px-5 py-3" style={{ background: '#111e34' }}>
...
export function DashboardCarregando() {
  return (
    <div aria-busy="true" aria-label="Carregando dashboard" className="min-h-screen flex flex-col">
      <CabecalhoCarregando />
      <div className="flex flex-1 flex-col md:flex-row">
        <aside className="w-full md:w-72 shrink-0 border-b md:border-b-0 md:border-r border-[#1a2138] p-3 space-y-2" style={{ background: '#070b18' }}>
        ...
        <main className="flex-1 min-w-0 p-4 sm:p-6 space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[0, 1, 2, 3].map((indice) => <Bloco key={indice} className="h-28 rounded-xl" />)}
          </div>
          <Bloco className="h-72 w-full rounded-xl" />
        </main>
      </div>
    </div>
  )
...
export function PlanilhaCarregando() {
  return (
    <div aria-busy="true" aria-label="Carregando planilha" className="min-h-screen flex flex-col">
      <CabecalhoCarregando />
      <main className="flex-1 w-full max-w-[1400px] mx-auto p-4 sm:p-6 space-y-5">
        ...
        <div className="rounded-xl border border-borda bg-superficie overflow-hidden p-4 space-y-3">
